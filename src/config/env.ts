export const parseEnvToConfig = (env: Record<string, string | undefined>) => {
  const partial: any = {};
  ...
  Object.entries(env).forEach(([k, v]) => {
    ...
    const num = Number(v);
    partial[category][camelLeaf] = isNaN(num) ? v : num;
  });

  return partial;
};

export const parseImportMetaEnv = (importMetaEnv: any) => parseEnvToConfig(importMetaEnv);
