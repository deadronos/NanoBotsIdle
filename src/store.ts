...
export const useGameStore = create<GameState>((set, get) => ({
  ...
  getUpgradeCost: (type) => {
    const state = get();
    switch(type) {
        case 'drone': return Math.floor(BASE_COSTS.drone * Math.pow(1.5, state.droneCount - 3));
        case 'speed': return Math.floor(BASE_COSTS.speed * Math.pow(1.3, state.miningSpeedLevel - 1));
        case 'move': return Math.floor(BASE_COSTS.move * Math.pow(1.3, state.moveSpeedLevel - 1));
        case 'laser': return Math.floor(BASE_COSTS.laser * Math.pow(1.4, state.laserPowerLevel - 1));
    }
    return 999999;
  },
