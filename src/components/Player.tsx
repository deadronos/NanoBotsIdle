...
export const Player: React.FC<PlayerProps> = ({ viewMode }) => {
  ...
  useEffect(() => {
    ...
    const handleMouseMove = (e: MouseEvent) => {
      if (document.pointerLockElement === document.body) {
        ...
        // Clamp pitch
        cameraAngle.current.pitch = Math.max(-Math.PI / 2 + 0.1, Math.min(Math.PI / 2 - 0.1, cameraAngle.current.pitch));
      }
    };
  ...
  useFrame((state, delta) => {
    ...
    const forward = new Vector3(0, 0, -1).applyAxisAngle(new Vector3(0, 1, 0), cameraAngle.current.yaw);
