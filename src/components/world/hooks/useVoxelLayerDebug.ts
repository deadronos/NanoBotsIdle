...
export function useVoxelLayerDebug({
  chunkSize,
  prestigeLevel,
}: UseVoxelLayerDebugProps) {
  const cfg = useConfig();
  ...
  const updateDebugBounds = useCallback((cx: number, cy: number, cz: number) => {
    if (debugEnabled) {
  ...
  const trackFrontierAdd = useCallback((positions: Float32Array | number[]) => {
  ...
  const trackFrontierRemove = useCallback((positions: Float32Array | number[]) => {
    if (!debugEnabled) return;
    for (let i = 0; i < positions.length; i += 3) {
  ...
  const logDebugInfo = useCallback((pcx: number, pcy: number, pcz: number, frame: { delta: RenderDelta }) => {
    if (!debugEnabled) return;
