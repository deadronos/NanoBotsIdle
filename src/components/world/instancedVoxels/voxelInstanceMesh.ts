...
export const rebuildVoxelInstances = (mesh: InstancedMesh, tmp: Object3D, positions: number[]) => {
  const count = Math.floor(positions.length / 3);
  for (let i = 0; i < count; i += 1) {
    const base = i * 3;
    setVoxelInstance(mesh, tmp, i, positions[base], positions[base + 1], positions[base + 2]);
  }
