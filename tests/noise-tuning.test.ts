...
const computeAboveWaterFraction = (
  ...
  updateConfig({ terrain: { noiseType, surfaceBias, quantizeScale } });
...
test("tune open-simplex to match sincos above-water fraction", () => {
  const cfg = getConfig();
  const baseline = computeAboveWaterFraction(
    "sincos",
  ...
  for (const b of biases) {
    for (const s of scales) {
      ...
      console.log(
        `open-simplex bias=${b}, scale=${s} => ${(frac * 100).toFixed(2)}% (diff ${(diff * 100).toFixed(2)}%)`,
      );
  ...
  console.log(
    `best candidate: bias=${best!.bias}, scale=${best!.scale}, frac=${(best!.frac * 100).toFixed(2)}% (diff ${(best!.diff * 100).toFixed(2)}%)`,
  );
